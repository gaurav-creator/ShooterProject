...
void UALSCharacterAnimInstance::NativeUpdateAnimation(float DeltaSeconds)
{
	...
	{
		LeftHandTransform = EquippedWeapon->GetWeaponMesh()->GetSocketTransform(FName("LeftHandSocket"), ERelativeTransformSpace::RTS_Component);

		if (Character->IsLocallyControlled())
		{
			bLocallyControlled = true;
			
			/*FTransform RightHandTransform = Character->GetMesh()->GetSocketTransform(FName("hand_r"), ERelativeTransformSpace::RTS_Component);
			RightHandRotation = UKismetMathLibrary::FindLookAtRotation(RightHandTransform.GetLocation(),
																				   RightHandTransform.GetLocation() + (
																					   RightHandTransform.GetLocation() - Character->
																					   GetHitTarget()));*/
		}
...
////////////////////////////////////
///
void UALSCharacterAnimInstance::IF_Recoil_Implementation(float Strength, float Delay, float TimeBeforeStart)
{
